...
for num in range(epoc_size):
    ...
    for i in range(pic_num//batch_size):
        ...
        X2_sub=ramp.ReLU(Y1)
        X2_list,ignores=dp.drop(X2_sub,rho)
        ...
        en_dp=dp.drop_bp(X2_list,en_x2,ignores)
        en_ramp=ramp.ReLU_bp(Y1,en_dp)
        _,en_w1,en_b1=dif.diff(en_ramp,W1,X1_list)
...
"""
#最終的な重みをファイルに保存
np.save("W1_issueA2.npy", W1)
np.save("b1_issueA2.npy", b1)
np.save("W2_issueA2.npy", W2)
np.save("b2_issueA2.npy", b2)
"""

#クロスエントロピー誤差を記録
np.save("entropy_dropout_ReLU.npy", entropy)
